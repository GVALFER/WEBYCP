import type { components } from "./schema";

export type HealthResponse = components["schemas"]["HealthResponse"];
export type LoginRequest = components["schemas"]["LoginRequest"];
export type AuthResponse = components["schemas"]["AuthResponse"];
export type UpdateProfileRequest = components["schemas"]["UpdateProfileRequest"];
export type Account = components["schemas"]["Account"];
export type AccountOverview = components["schemas"]["AccountOverview"];
export type Pagination = components["schemas"]["Pagination"];
export type AccountListResponse = components["schemas"]["AccountListResponse"];
export type AccountJobResponse = components["schemas"]["AccountJobResponse"];
export type CreateAccountRequest = components["schemas"]["CreateAccountRequest"];
export type Package = components["schemas"]["Package"];
export type PackageLimits = components["schemas"]["PackageLimits"];
export type PackageListResponse = components["schemas"]["PackageListResponse"];
export type WritePackageRequest = components["schemas"]["WritePackageRequest"];
export type Website = components["schemas"]["Website"];
export type WebsiteListResponse = components["schemas"]["WebsiteListResponse"];
export type WebsiteJobResponse = components["schemas"]["WebsiteJobResponse"];
export type WebsiteDomain = components["schemas"]["WebsiteDomain"];
export type WebsiteDomainListResponse = components["schemas"]["WebsiteDomainListResponse"];
export type WebsiteDomainJobResponse = components["schemas"]["WebsiteDomainJobResponse"];
export type Node = components["schemas"]["Node"];
export type NodeListResponse = components["schemas"]["NodeListResponse"];
export type ServiceCapability = components["schemas"]["ServiceCapability"];
export type ServiceCapabilities = components["schemas"]["ServiceCapabilities"];
export type ServiceDefaults = components["schemas"]["ServiceDefaults"];
export type ServiceSettings = components["schemas"]["ServiceSettings"];
export type Job = components["schemas"]["Job"];
export type JobListResponse = components["schemas"]["JobListResponse"];
export type JobDetail = components["schemas"]["JobDetail"];
export type AuditEventListResponse = components["schemas"]["AuditEventListResponse"];
export type ScheduledTask = components["schemas"]["ScheduledTask"];
export type FTPAccount = components["schemas"]["FTPAccount"];
export type FTPAccountListResponse = components["schemas"]["FTPAccountListResponse"];
export type FTPAccountResponse = components["schemas"]["FTPAccountResponse"];
export type CertificateListResponse = components["schemas"]["CertificateListResponse"];
export type CertificateJobResponse = components["schemas"]["CertificateJobResponse"];
export type DatabaseListResponse = components["schemas"]["DatabaseListResponse"];
export type DatabaseUserListResponse = components["schemas"]["DatabaseUserListResponse"];
export type DatabaseGrantListResponse = components["schemas"]["DatabaseGrantListResponse"];
export type DatabaseJobResponse = components["schemas"]["DatabaseJobResponse"];
export type DatabaseUserJobResponse = components["schemas"]["DatabaseUserJobResponse"];
export type DatabaseGrantJobResponse = components["schemas"]["DatabaseGrantJobResponse"];
export type ScheduledTaskListResponse = components["schemas"]["ScheduledTaskListResponse"];
export type ScheduledTaskResponse = components["schemas"]["ScheduledTaskResponse"];
export type WriteScheduledTaskRequest = components["schemas"]["WriteScheduledTaskRequest"];
export type BackupPlan = components["schemas"]["BackupPlan"];
export type BackupArtifact = components["schemas"]["BackupArtifact"];
export type BackupPlanListResponse = components["schemas"]["BackupPlanListResponse"];
export type BackupRunListResponse = components["schemas"]["BackupRunListResponse"];
export type BackupArtifactListResponse = components["schemas"]["BackupArtifactListResponse"];
export type BackupRunResponse = components["schemas"]["BackupRunResponse"];
export type BackupManifest = components["schemas"]["BackupManifest"];
export type RestoreBackupRequest = components["schemas"]["RestoreBackupRequest"];
export type WriteBackupPlanRequest = components["schemas"]["WriteBackupPlanRequest"];
export type DNSProvider = components["schemas"]["DNSProvider"];
export type DNSProviderListResponse = components["schemas"]["DNSProviderListResponse"];
export type DNSSettings = components["schemas"]["DNSSettings"];
export type UpdateDNSSettingsRequest = components["schemas"]["UpdateDNSSettingsRequest"];
export type DNSZone = components["schemas"]["DNSZone"];
export type DNSZoneListResponse = components["schemas"]["DNSZoneListResponse"];
export type DNSZoneJobResponse = components["schemas"]["DNSZoneJobResponse"];
export type DNSRecord = components["schemas"]["DNSRecord"];
export type DNSRecordListResponse = components["schemas"]["DNSRecordListResponse"];
export type DNSRecordJobResponse = components["schemas"]["DNSRecordJobResponse"];
export type WriteDNSRecordRequest = components["schemas"]["WriteDNSRecordRequest"];
