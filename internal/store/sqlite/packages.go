...
func requireCapacity(ctx context.Context, tx *sql.Tx, accountID string, kind limitKind) error {
	queries := [...]string{
		...
		`SELECT packages.max_scheduled_tasks, (SELECT COUNT(*) FROM cron_jobs WHERE cron_jobs.account_id = accounts.id) FROM accounts JOIN account_packages ON account_packages.account_id = accounts.id JOIN packages ON packages.id = account_packages.package_id WHERE accounts.id = ?`,
